...
def create_bot_config(pairs):
    ...
    for pair in pairs:
        ...
        # Check if either token in the pair is in the unsupported list, if so skip this pair.
        if token0['id'].lower() in unsupported_tokens or token1['id'].lower() in unsupported_tokens:
            continue

        # Check if token0 or token1 is WETH, if so replace with ETH
...
# Generate config for top 100 pairs
