...
async function uniswap(req, res) {
    ...
    const quote = await uniswapV2.getQuote(BigNumber.from(1), tokenA, tokenB);
...
async function fakecex(req, res) {
    ...
    const quote = await fakeCex.getQuote(BigNumber.from(1), tokenA, tokenB);
...
async function livecex(exchange, req, res) {
    ...
    const quote = await liveCex.getQuote(BigNumber.from(1), tokenA, tokenB);
