...
public:
  ...
  BigInteger reductionBarrett(const BigInteger &mod,
  ...
  BigInteger powmodBarrett(const BigInteger &power,
                           const BigInteger &mod) const;
  ...
  BigInteger expRightToLeft(const BigInteger &power,
                            const BigInteger &mod) const;

  vector<BigInteger>
  expLeftToRightK_aryPrecomputation(const BigInteger &mod) const;
  BigInteger expLeftToRightK_ary(const BigInteger &exponent,
                                 const BigInteger &mod,
                                 const vector<BigInteger> &g) const;

  vector<BigInteger> expLeftToRightK_aryVarPrecomputation(const BigInteger &mod,
                                                          size_t k) const;
  BigInteger expLeftToRightK_aryVar(const BigInteger &, const BigInteger &,
                                    const vector<BigInteger> &, size_t k) const;

  vector<BigInteger>
  expLeftToRightK_aryModifPrecomputation(const BigInteger &) const;
  BigInteger expLeftToRightK_aryMod(const BigInteger &, const BigInteger &,
                                    const vector<BigInteger> &) const;

  // Slide operations
  vector<BigInteger> expSlidingWindowPrecomputation(const BigInteger &,
                                                    size_t k) const;
  BigInteger expSlidingWindow(const BigInteger &, const BigInteger &,
                              const vector<BigInteger> &, size_t k) const;

  // for best result:
  vector<BigInteger> expBest_SlidePrecomp(const BigInteger &mod) const;
  BigInteger expBest_Slide(const BigInteger &exponent, const BigInteger &mod,
                           const vector<BigInteger> &g) const;
