...
constexpr bt4 max_size_fast_mul =
    numeric_limits<bt4>::max() / bmax / bmax / bmax * (bmax - 1) - 1;
...
BN::BN(BN &&bn) noexcept : data(std::move(bn.data)) {}

BN::BN(vector<bt> &&v) noexcept : data(std::move(v)) { norm(data); }

BN::BN(const vector<bt> &v) : data(v) { norm(data); }

BN::BN(vector<bt> &&v, size_t rbc) : data(std::move(v)) { data.resize(rbc); }
...
BN::BN(const string &str, RadixBI radix) {
  ...
  for (size_t i = 0; i < length; ++i) {
    ...
    if (str[i] >= '0' && str[i] <= '9')
      ...
      throw invalid_argument(string("BN constructor: invalid char '") + str[i] +
                             "' in HEX string");
...
void BN::swap(BN &bn) noexcept { data.swap(bn.data); }
...
BN BN::divbt(size_t t) const {
  ...
  return std::move(vector<bt>(data.begin() + t, data.end()));
}
...
BN BN::modbt(size_t t) const {
  ...
  return std::move(vector<bt>(data.begin(), data.begin() + t));
}

// Base multiplciation
const BN BN::mulbase(const bt &multiplier) const {
  return std::move(BN(*this).mulbaseappr(multiplier));
}
...
const BN BN::divbase(const bt &divider) const {
  return std::move(BN(*this).divbaseappr(divider));
}
...
const BN BN::operator*(const BN &bn) const { return karatsuba_mul(bn); }
...
inline vector<bt> karatsubaSum(const vector<bt> &u, size_t start, size_t n,
                               size_t m) {
  vector<bt> result;
  result.reserve(m + 1);
...
vector<bt> karatsubaRecursive(const vector<bt> &U, const vector<bt> &V,
  ...
  if (n < karatsuba_min_size) {
    ...
    for (size_t s = 0; s < len + len - 1; s++) {
      ...
      for (size_t i = start_index + start, j = s - start_index + start;
           i <= end_index; i++, --j)
  ...
  vector<bt> A = std::move(karatsubaRecursive(U, V, start + n, m));
  vector<bt> B = std::move(karatsubaRecursive(U, V, start, n));
  const vector<bt> C = std::move(karatsubaRecursive(u01, v01, 0, m + 1));
...
void BN::divmod(const BN &bn, BN &div, BN &mod) const {
  ...
  if (bn.data.size() == 1) {
    div = std::move(this->divbase(bn.data.front()));
    mod = std::move(this->modbase(bn.data.front()));
    return;
  }
  ...
  for (size_t j = m; j <= m; --j) {
    bt2 q = (dividend.data[j + n] * bsize + dividend.data[j + n - 1]) /
            divider.data[n - 1];
    bt2 r = (dividend.data[j + n] * bsize + dividend.data[j + n - 1]) %
            divider.data[n - 1];

    if (q == bsize ||
        q * divider.data[n - 2] > bsize * r + dividend.data[j + n - 2]) {
      --q;
      r += divider.data[n - 1];
      if (r < bsize &&
          q * divider.data[n - 2] > bsize * r + dividend.data[j + n - 2])
...
bool BN::operator>(const BN &bn) const noexcept { return bn < *this; }

bool BN::operator<=(const BN &bn) const noexcept { return !(*this > bn); }

bool BN::operator>=(const BN &bn) const noexcept { return !(*this < bn); }

bool BN::operator==(const BN &bn) const noexcept { return data == bn.data; }

bool BN::operator!=(const BN &bn) const noexcept { return !(*this == bn); }

bt BN::operator[](size_t index) const noexcept { return data[index]; }

// Digit information

size_t BN::digit_count() const noexcept { return data.size(); }
...
BN &BN::operator>>=(size_t shift) {
  ...
  for (size_t i = 0; i < data.size() - baseshift - 1; ++i) {
    data[i] = (data[i + baseshift] >> realshift) |
...
BN &BN::operator<<=(size_t shift) {
  ...
  for (size_t i = data.size() - 1; i; --i) {
    data[i + baseshift] =
        (data[i - 1] >> (bz8 - realshift)) | (data[i] << realshift);
  }
  ...
  for (size_t i = 0; i < data.size(); ++i) {
    ...
    for (size_t j = i + 1; j < data.size(); ++j) {
      cuv = static_cast<bt4>(res.data[i + j]) +
            ((static_cast<bt4>(data[i]) * data[j]) << 1) + (cuv >> bz8);
      res.data[i + j] = cuv;
    }
...
BN BN::expLeftToRightK_ary(const BN &exponent, const BN &mod,
                           const vector<BN> &g) const {
  if (exponent.isZero())
    return BN::BigInteger1();
...
vector<BN> BN::expLeftToRightK_aryVarPrecomputation(const BN &mod,
                                                    size_t K) const {
  int Kmax = (1 << K);
  BN g = *this % mod;
...
BN BN::expLeftToRightK_aryVar(const BN &exponent, const BN &mod,
                              const vector<BN> &g, size_t K) const {
  if (exponent.isZero())
    return BN::BigInteger1();
...
BN BN::expLeftToRightK_aryMod(const BN &exponent, const BN &mod,
                              const vector<BN> &g) const {
  if (exponent.isZero())
    return BN::BigInteger1();
...
BN BN::expSlidingWindow(const BN &exponent, const BN &mod, const vector<BN> &g,
                        size_t K) const {
  BN A(BN::BigInteger1());
...
BN BN::expBest_Slide(const BN &exponent, const BN &mod,
                     const vector<BN> &g) const {
  BN A(BN::BigInteger1());
  BN mu = reductionBarrettPrecomputation(mod);
  int i = exponent.bit_count() - 1;
...
uint64_t BN::get64() const noexcept {
  uint64_t result = 0;
  for (size_t i = min(data.size(), sizeof(uint64_t) / bz) - 1;
       i < sizeof(uint64_t); i--)
    result = (result << bz8) | data[i];
  return result;
}

const vector<bt> BN::raw() const noexcept { return data; }
...
BN BN::random(size_t byteCount) {
  ...
  return std::move(BN(std::move(result)));
}
